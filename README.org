...
[[https://github.com/benkio/sBots/actions/workflows/scalaCI.yml/badge.svg][https://github.com/benkio/sBots/actions/workflows/scalaCI.yml/badge.svg]]
...
** Bots
     ...
     | https://t.me/PinoScottoBot       | [[https://it.wikipedia.org/wiki/Pino_Scotto][Pino Scotto]] |
     | https://t.me/XahLeeBot           | [[http://xahlee.info/][Xah Lee]] |
     | https://t.me/CalandroBot         | |

** Adding a new bot

  Step-by-step guide: [[docs/adding-a-bot.md][Adding a new bot]] (template, build registration, CI token secret, and README update).
...
**** GUI tool (Replies Editor)

     To edit a bot’s ~*_replies.json~ via a GUI (and auto-regenerate ~*_triggers.txt~ on save), use the [[modules/repliesEditorServer/README.md][Replies Editor (GUI)]].

#+begin_src sh
  sbt repliesEditorServer/run
#+end_src
...
**** Localhost
     ...
     - run ~sbt assembly~
     - run ~java -cp main/target/scala-3.7.2/main.jar com.benkio.main.MainWebhook~
...
** Healthcheck
  ...
  monitor that the service is up (e.g. [[https://healthchecks.io][healthchecks.io]]).

  - **Where**: The healthcheck runs in the main module; one background
    fiber pings the configured URL on a cron schedule.
  - **Config**: In ~modules/main/src/main/resources/application.conf~
    under ~main.healthcheck-ping~:
    - ~endpoint~ :: URL to GET (e.g. ~https://hc-ping.com/<your-check-uuid>~).
    - ~cron~ :: Cron4s expression for how often to ping (e.g. ~* */10 * ? * *~ for every 10 minutes).
  - **Overrides**: You can override with env vars if your config
    supports it; otherwise edit ~application.conf~ before assembly.
  - **Note**: Only one healthcheck fiber is started per process. If you
    run multiple replicas, each will ping the same endpoint on each
    tick.
...
*** Deploy
 ...
 - Move the fat jar to the VM by ~rsync~ and ~ssh~. Eg. ~rsync -P -e "ssh -i <<private key of the vm>>" <<path to the fat jar -> sBots/main/target/scala-3.7.2/main.jar>> <<user of the vm>>@<<public IP of the vm>>:/home/<<user of the vm>>/main.jar~ (~rsync -P -e "ssh -i ubuntu_rsa.pem" /home/benkio/workspace/sBots/main/target/scala-3.7.2/main.jar ubuntu@129.152.27.196:/home/ubuntu/bots/main.jar~)
